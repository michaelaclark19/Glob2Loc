...
targets <- read.csv(paste0(getwd(),"/Land Forecasting Targets/Land_Targets_2August2021.csv"),
                    header = TRUE,
...
if('Land Forecast Outputs' %in% list.files(getwd())) {
  # Nothing
  } else {
  # Create directory
  dir.create(paste0(getwd(),'/Land Forecast Outputs'))
  dir.create(paste0(getwd(),'/Land Forecast Outputs/BAU'))
  dir.create(paste0(getwd(),'/Land Forecast Outputs/BAU/tifs'))
  }
...
for(k in (1:12)) { # Odd ordering is to complete the small regions first to check the script
#for(k in 1) {
      	cat(region.names[k])	
  ...
  completed_countries <- list.files(path = paste0(getwd(),"/Land Forecast Outputs/BAU/tifs"))
  ...
  targets.tmp = targets.tmp[!is.na(targets.tmp$ISO3) & targets.tmp$scenario == "BAU",]
...
#  iso3c <- iso3c[iso3c %in% countries.keep]
  ...
  if(length(iso3c) == 0){
    ...
    #for(c in 4) {
      ...
      targets <- subset(targets, scenario == "BAU")
	...
	#print(tmp.length)
      ...
      for(i in 1:length(raster.list.crop)) {
        ...
        writeRaster(raster.list.crop[[i]],
                    paste0(getwd(),"/Land Forecast Outputs/BAU/tifs/",
                          iso3c[c],
                          region.names[k],
                          names(raster.list.crop)[i],
                          "_BAU_Crop.tif"),

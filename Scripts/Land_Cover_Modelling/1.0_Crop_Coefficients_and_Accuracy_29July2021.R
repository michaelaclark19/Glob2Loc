...
file.names.save <- paste0(region_list, "_29July2021_Cutoff_025_Crop.rda")
...
for (k in c(1:12)) {
  ...
  dat_orig <- 
    ...
    dplyr::rename(dist_50k = distance_cities,
                  ...
                  PA_binary = wdpa,
                  prop_to_predict_from_cell_delta_crop = delta_crop_2000_2005) %>%
  ...
  dat_orig[,'has_changed_crop'] <- NA
  dat_orig[which(dat_orig[,'delta_crop_2005_2010'] >= cutoff  & !is.na(dat_orig[,'delta_crop_2005_2010'])),'has_changed_crop'] <- 'Increase'
  dat_orig[which(dat_orig[,'delta_crop_2005_2010'] <= -cutoff  & !is.na(dat_orig[,'delta_crop_2005_2010'])),'has_changed_crop'] <- 'Decrease'
  dat_orig[which(is.na(dat_orig[,'has_changed_crop']) & !is.na(dat_orig[,'prop_to_predict_from_cell_delta_crop'])),'has_changed_crop'] <- 'aNoChange'
  ...
  dat_orig <- dplyr::select(dat_orig,
                            ...
                            prop_adj_to_predict_from_crop = adj_Crop2005,
                            prop_to_predict_from_crop = Crop2005,
                            prop_to_predict_from_cell_delta_crop = prop_to_predict_from_cell_delta_crop,
                            crop_extent_change_2010.2005 = delta_crop_2005_2010,
                            prop_to_predict_from_pasture = Pasture2005,
                            prop_crop_cell_2000 = Crop2000,
                            prop_crop_cell_2010 = Crop2010,
                            prop_change_cell_crop_2005.2010 = delta_crop_2005_2010,
                            has_changed_crop = has_changed_crop)
  ...
  dat_mod <- dat_orig[,c('dist_50k','ag_suitability','PA_binary','prop_adj_to_predict_from_crop','prop_to_predict_from_crop','prop_to_predict_from_pasture',
                         'prop_to_predict_from_cell_delta_crop',"has_changed_crop", "prop_change_cell_crop_2005.2010","ISO_numeric")]
  ...
  dat_mod$prop_cell_delta_crop_sq <- dat_mod$prop_to_predict_from_cell_delta_crop^2
  dat_mod$prop_adj_crop_sq <- dat_mod$prop_adj_to_predict_from_crop^2
  ...
  outcomeName <- "has_changed_crop" # outcome factor variable
  # Attaching data set
  ...
  sample.size = min(table(dat_mod$has_changed_crop))
  
  
  dat_mod_new <-
    rbind(sample_n(dat_mod %>% filter(has_changed_crop %in% 'aNoChange'),sample.size),
          sample_n(dat_mod %>% filter(has_changed_crop %in% 'Decrease'),sample.size),
          sample_n(dat_mod %>% filter(has_changed_crop %in% 'Increase'),sample.size))
  ...
  for(l in 1:5) {
    ...
    # Getting multinomial regression model
    objModel <- multinom(has_changed_crop ~ dist_50k_log +
                           dist_50k_log_sq +
                           ag_suitability +
                           PA_binary +
                           prop_adj_to_predict_from_crop +
    ...
    dat.out <-
      dat.out %>%
      transform(has_changed_crop = factor(has_changed_crop, levels = c('aNoChange','Decrease','Increase'))) %>%
    ...
    accuracy <- caret::confusionMatrix(data = dat.out$predicted_values,
                                       reference = dat.out$has_changed_crop)
  ...
  acc.sum <-
    dat.out %>%
    group_by(predicted_values, has_changed_crop) %>%
    dplyr::summarise(Count = n()) %>%
    left_join(.,
              dat_mod %>% group_by(has_changed_crop) %>% dplyr::summarise(Count_Actual = n())) %>%
    mutate(plot.x = ifelse(has_changed_crop %in% 'aNoChange',2,
                           ifelse(has_changed_crop %in% 'Decrease',1,3))) %>%
  ...
  ggplot(acc.sum) +
    ...
    coord_flip() +
    labs(x = 'Actual Cropland Change', y = 'Predicted Cropland Change', fill = 'Percent Accuracy') +
  ...
  ggsave(paste0(getwd(),'/Land Forecasting Coefs and Plots/Model Plots/Crop Multinomial Accuracy 29July2021 ',
  ...
  dat_region_increase <- dat_mod[dat_mod$has_changed_crop %in% 'Increase',]
  dat_region_decrease <- dat_mod[dat_mod$has_changed_crop %in% 'Decrease',]
  
  # removing data
  rm(dat_mod)
  
  # Dropping rows with values <0 for increaes and > 0 for decrease
  dat_region_increase <- dat_region_increase[dat_region_increase$prop_change_cell_crop_2005.2010 > 0,]
  dat_region_decrease <- dat_region_decrease[dat_region_decrease$prop_change_cell_crop_2005.2010 < 0,]
  ...
  for (l in 1:5) {
    ...
    objModel_increase <- glm(prop_change_cell_crop_2005.2010 ~ 
                               dist_50k_log +
                               dist_50k_log_sq +
                               ag_suitability +
                               PA_binary +
    ...
    # Getting rmse
    rmse.list[[l]] <- RMSE(obs = test.dat$prop_change_cell_crop_2005.2010[test.dat$ISO_numeric %in% train.dat$ISO_numeric], # Observed values
    ...
    iso3s = levels(test.dat$ISO_numeric)[levels(test.dat$ISO_numeric) %in% unique(test.dat$ISO_numeric)]
    for(i in iso3s) {
  ...
  dat_region_decrease$prop_change_cell_crop_2005.2010 <- abs(dat_region_decrease$prop_change_cell_crop_2005.2010)
  ...
  for (l in 1:5) {
    ...
    test.dat <-
      ...
      filter(group %in% l)
    # Getting model for extent increase
    objModel_decrease <- glm(prop_change_cell_crop_2005.2010 ~ 
                               dist_50k_log +
                               dist_50k_log_sq +
                               ag_suitability +
                               PA_binary +
    ...
    # Predicting normal values
    test.dat$preds <- exp(predict(objModel_decrease, test.dat))
  ...
  dat.out = data.frame()
  tmp.coefs = out.weights
  
  iso3s = levels(dat_region_decrease$ISO_numeric)[levels(dat_region_decrease$ISO_numeric) %in% unique(dat_region_decrease$ISO_numeric)]
  for(i in iso3s) {
    dat.out <- rbind(dat.out,
                     dat_region_decrease %>% filter(ISO_numeric %in% i) %>%
                       mutate(pred_change_cell_prop_crop_decrease =
                                glm.predict.2b(dat = dat_region_decrease %>% filter(ISO_numeric %in% i),
                                               coefs = tmp.coefs,
                                               vars = predictor_vars_crop[1:(length(predictor_vars_crop) - 1)],
                                               factor_vars = 'ISO_numeric')))
  ...
  # Saving coefficients
  # write.csv(coef.multinom.mean,paste0(getwd(),'/Land Forecasting Coefs and Plots/Model Coefficients/',region_list[k],' Crop Multinomial Mean Coefficients.csv'), row.names = FALSE)
  # write.csv(coef.multinom.se,paste0(getwd(),'/Land Forecasting Coefs and Plots/Model Coefficients/',region_list[k],' Crop Multinomial se Coefficients.csv'), row.names = FALSE)
  # write.csv(coef.increase.mean.se,paste0(getwd(),'/Land Forecasting Coefs and Plots/Model Coefficients/',region_list[k],' Crop GLM Increase Coefficients.csv'), row.names = FALSE)
  # write.csv(coef.decrease.mean.se,paste0(getwd(),'/Land Forecasting Coefs and Plots/Model Coefficients/',region_list[k],' Crop GLM Decrease Coefficients.csv'), row.names = FALSE)
  
  # And saving multinomial accuracy
  save(file = paste0(getwd(),'/Land Forecasting Coefs and Plots/Multinomial Accuracy/',region_list[k],' Crop Multinomial Accuracy 29July2021.RData'),
       accuracy)
  
  save(file = paste0(getwd(),'/Land Forecasting Coefs and Plots/Model Coefficients/',region_list[k],' Crop Model Coefficients 29July2021.RData'),
       coef.multinom.mean, coef.increase.mean.se, coef.decrease.mean.se, coef.multinom.se)

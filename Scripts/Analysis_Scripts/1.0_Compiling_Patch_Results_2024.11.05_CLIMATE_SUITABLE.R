...
species_loop_function <-
  function(ss) {
    ...
    write.csv(out_df_nomigrate,
              paste0(files_write_wd,'/Outputs/Managed_CSV_Files/Abs_Impacts/',tt,'/',ss,'_Climate_Suitable_nopatchmigration_',thresh,'.csv'),
              row.names = FALSE)
    ...
    write.csv(out_df_nomigrate_prop,
              paste0(files_write_wd,'/Outputs/Managed_CSV_Files/Prop_Impacts/',tt,'/',ss,'_Climate_Suitable_nopatchmigration_',thresh,'.csv'),
              row.names = FALSE)
...
# Making function to catch errors if needed...
try_catch_function <-
  function(ss) {
...
for(thresh in threshold_list) {
  ...
  for(tt in taxa_list) {
    ...
    taxa_files <-
      list.files(paste0(files_stored_wd,'/Outputs/CSV_File_Outputs/BAU/',tt),
                 full.names = TRUE,
                 pattern = 'Spatial_Population') %>%
      #.[!grepl('nomigration',.)] %>%
      .[grepl(tt,.)] %>%
    ...
    species_list_2020 <-
      taxa_files %>%
      gsub('Climate_Suitable_Spatial_Population_Estimates_csv_','',.) %>%
      .[grepl('2020',.)] %>%
      gsub('_[0-9]{4,4}.*','',.) %>%
      gsub(paste0('.*',tt,'/'),'',.) %>%
      unique()
    ...
    # Species we have
    species_have <-
      list.files(paste0(files_write_wd,'/Outputs/Managed_CSV_Files/Prop_Impacts/',tt,'/')) %>%
      .[grepl(thresh,.)] %>%
      .[grepl('Climate_Suitable',.)] %>%
      gsub('_nopatch.*','',.) %>%
    ...
    #mclapply(species_list, species_loop_function, mc.cores = 10)

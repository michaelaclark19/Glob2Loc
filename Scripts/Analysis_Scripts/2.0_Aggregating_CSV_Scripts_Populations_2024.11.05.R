...
# working directory
setwd('/data/ouce-glob2loc/pubh0329/Multiple_Stresses_of_Biodiversity/Outputs/Managed_CSV_Files_Migration/')
...
for(thresh in 'specsens') {
  # looping through taxa types
  for(tt in taxa_list[3]) {
...
names_df <- read_csv(file_list[1])
     ...
     out_df_nomigrate <- do.call(rbind, mclapply(file_list, read_csv_function, mc.cores = 10))
      ...
      write.csv(out_df_nomigrate,
                paste0(gsub('/Managed_CSV_Files','/Aggregated_CSV_Files',getwd()),'/',ii,'_',tt,'_',thresh,'_nopatchmigration.csv'),
                row.names = FALSE)

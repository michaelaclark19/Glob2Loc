...
already_have_tmp <-
        list.files(paste0(getwd(),'/Analyses//Temp_Folder_Species_By_Ecoregion/'),
                   full.names = TRUE) %>%
.[grepl(mm,.)] %>%
.[grepl(thresh,.)] %>%
.[grepl(tt,.)] %>%
gsub('.*Temp_Folder_Species_By_Ecoregion//','',.) %>%
gsub(paste0('_',mm,'.*'),'',.)
...
dir.create(paste0(getwd(),'/Analyses/Temp_Folder_Species_By_Ecoregion'))

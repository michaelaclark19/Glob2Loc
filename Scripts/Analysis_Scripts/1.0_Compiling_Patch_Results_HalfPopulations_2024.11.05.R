...
species_loop_function <-
  function(ss) {
   ...
   out_df_nomigrate <-
	  out_df_nomigrate %>%
	 mutate(pop_dispersal = 'half_patch_migration') 
 ...
 out_df_nomigrate_prop <-
    ...
    write.csv(out_df_nomigrate,
              paste0(files_write_wd,'/Outputs/Managed_CSV_Files_Migration/Abs_Impacts/',tt,'/',ss,'_half_patch_migration_',thresh,'.csv'),
              row.names = FALSE)
    ...
    write.csv(out_df_nomigrate_prop,
              paste0(files_write_wd,'/Outputs/Managed_CSV_Files_Migration/Prop_Impacts/',tt,'/',ss,'_half_patch_migration_',thresh,'.csv'),
              row.names = FALSE)

...
species_loop_function <-
  function(ss) {
    ...
    write.csv(out_df_nomigrate,
              paste0(files_write_wd,'/Outputs/Managed_CSV_Files_EAT_Lancet/Abs_Impacts/',tt,'/',ss,'_nopatchmigration_',thresh,'.csv'),
              row.names = FALSE)
    ...
    write.csv(out_df_nomigrate_prop,
              paste0(files_write_wd,'/Outputs/Managed_CSV_Files_EAT_Lancet/Prop_Impacts/',tt,'/',ss,'_nopatchmigration_',thresh,'.csv'),
              row.names = FALSE)
...
for(thresh in threshold_list) {
  ...
  for(tt in taxa_list) {
    ...
    taxa_files <-
      list.files(paste0(files_stored_wd,'/Outputs/CSV_File_Outputs/EAT_Lancet/',tt),
                 full.names = TRUE,
                 pattern = 'Spatial_Population') %>%
      .[!grepl('nomigration',.)] %>%
      .[grepl(tt,.)] %>%
    ...
    # Species we have
    species_have <-
      list.files(paste0(files_write_wd,'/Outputs/Managed_CSV_Files_EAT_Lancet/Prop_Impacts/',tt,'/')) %>%
      .[grepl(thresh,.)] %>%
      .[!grepl('Climate_Suitable',.)] %>%
      gsub('_nopatch.*','',.) %>%

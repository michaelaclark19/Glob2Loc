...
coo.dat <- read.csv(paste0(getwd(),"/COO_Data/NewCountryOfOccurrenceData.csv"),stringsAsFactors = FALSE) %>%
  ...
  # filter(ISO3 %in% 'BRA') # UNCOMMENT IF RUNNING FOR ONLY SPECIFIC COUNTRIES
...
sdm_wrap <- function(k) {
  ...
  ###
  # CHECKING WHETHER SPECIES EXIST IN THE ECOREGION
  tmp_eco_check <-
    raster::crop(ecoregion_map_check,
                 esh.raster)
  
  # Unique values
  tmp_eco_check[is.na(esh.raster)] <- NA
  tmp_eco_values_continue <-
    unique(getValues(tmp_eco_check)) %>%
    .[!is.na(.)]
  
  if(sum(!(tmp_eco_values_continue %in% af_ecoregions)) == 0) {

...
dir.create(paste0(getwd(),'/ESH_RCPs/Raw/Reptiles'))
...
cat('Length ESH Reptiles',length(esh.reptiles),'\n')
...
coo.dat <- read.csv(paste0(getwd(),"/COO_Data/coo_dat_reptiles.csv"),stringsAsFactors = FALSE) %>%
...
cat('Rows in ESH.TOT, L227:',nrow(esh.tot),'\n')
...
if(length(already.have) > 0) {
  esh.tot <- 
    esh.tot %>%
    # filter(species_name %in% gsub('Reptiles/','',coo.dat$species)) %>%
...
cat('Rows in ESH.TOT, L256:',nrow(esh.tot),'\n')
...
cat('Rows in ESH.TOT, L287:',nrow(esh.tot),'\n')
### ---

...
auc.list <-
  ...
  .[!grepl('SSP1',.)] # Keeping the same AUC and Threshold as the SSP2s for safety. Seed was set, so these are identical, just making sure to not miss some species!
...
species.have <-
  c(list.files(paste0(getwd(),'/ESH_RCPs/SSP1-2.6/Amphibians'), full.names = TRUE),
    list.files(paste0(getwd(),'/ESH_RCPs/SSP1-2.6/Birds'), full.names = TRUE),
    list.files(paste0(getwd(),'/ESH_RCPs/SSP1-2.6/Mammals'), full.names = TRUE),
    list.files(paste0(getwd(),'/ESH_RCPs/SSP1-2.6/Reptiles'), full.names = TRUE)) %>%
  .[grepl('_SSP1.csv',.)] %>% 
  .[grepl('climate_suitable',.)] %>% 
  gsub('_climate_suitable_completed.*','',.) %>%
  gsub('.*SSP1-2.6/','',.)
...
species_wrap_fun <-
  function(i) {
        ...
        for(yy in years) {
          ...
          if(length(grep(tmp.name,auc.list)) %in% 0) {
            ...
            if(length(auc.list[grepl(tmp.name,auc.list)] %>%
              ...
              # Accuracy for weighting
              names(auc.csv)[names(auc.csv) %in% 'rf'] <- 'random.forest'
              auc.weight <- as.numeric(auc.csv[,names(models) %>% gsub('_SSP1','',.)])
              
              # Removing overfitting and underfitting models - This has been done earlier when creating the raw SDM rasters. Adding this here to be safe
              # Setting weight to .5, because weighting is calculated as (auc - .5)^2
              # This forces a weighting of 0 to be put on these models
              auc.weight[auc.weight >= 1] <- .5 
              auc.weight[auc.weight < .75] <- .5
              
              ###
              # Next getting model average based on auc values
              names(auc.weight) <- names(models) %>% gsub('_SSP1','',.)
              # Getting weighted model means
              # List of model aucs
              auc <- auc.weight
              # Weighting based on accuracy
    ...
    write.csv(data.frame(complete = 'complete'),
              paste0(getwd(),'/ESH_RCPs/SSP1-2.6/',tmp.taxa,'/',tmp.name,'_climate_suitable_completed_SSP1.csv'))  

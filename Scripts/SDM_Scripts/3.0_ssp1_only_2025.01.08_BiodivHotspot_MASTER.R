...
###
# Functions
source(paste0(getwd(),"/Scripts/SDM_Scripts/0.0_SDM_Functions_2024.07.13.R"))
...
if('ESH_RCPs' %in% list.files(getwd())) {
  ...
  for(i in dirs.to.create) {
    ...
    if(i != 'Mod_Accuracy') {
      sub.dirs <- paste0(i, c('/Amphibians','/Mammals','/Birds')) # Taxons in the analysis
      for(j in sub.dirs) {
...
# For species endemic to atlantic rain forest
ecoregion_map_check <- raster(paste0(getwd(),'/Ecoregions_Feb2023/TNC_Ecoregions_Map.tif'))

# List of species in each biodiversity hotspot
if('species_by_biodiv_hotspot_esh_maps.csv' %in%
   list.files(paste0(getwd(),'/Analyses/'))) {
  ...
  species_by_biodiv_hotspot <-
    ...
    distinct() %>%
    filter(prop_in_hotspot >.99)
  
  
} else {
  hotspot_files <- list.files(paste0(getwd(),'/Analyses/Temp_Folder_Species_By_Biodiv_Hotspot'),full.names = TRUE) %>% .[grepl('esh_map',.)]
  species_by_biodiv_hotspot <- 
    do.call(rbind,mclapply(hotspot_files,read_csv,mc.cores=10)) %>%
    # Some of the file names are a bit messed
    mutate(species = gsub("\\.tif$", "", species)) %>%
    mutate(species = gsub("(^.*/)(.*)(_HabElev.*$)", "\\2", species)) %>%
    mutate(species = gsub("_P.*$", "", species)) %>%
    mutate(species = gsub(".*/", "", species)) %>%
    mutate(species = gsub('_[A-Z].*','',species)) %>%
    mutate(species = gsub('_[0-9].*','',species))
  
  # Saving file
  write.csv(species_by_biodiv_hotspot,
       paste0(getwd(),'/Analyses/species_by_biodiv_hotspot_esh_maps.csv'),
       row.names = FALSE)
...
# Loading ESH maps -----
esh.mammals <-
  list.files(path = paste0(getwd(),"/ESH_Tifs_12Oct/Mammals"),
             full.names = TRUE)

esh.birds <-
  list.files(path = paste0(getwd(),"/ESH_Tifs_12Oct/Birds"),
             full.names = TRUE)

esh.amps <-
  list.files(path = paste0(getwd(),"/ESH_Tifs_12Oct/Amphibians"),
             full.names = TRUE)
esh.all <- c(esh.mammals, esh.birds, esh.amps)
taxon <- c(rep("Mammals", length(esh.mammals)),
           rep("Birds", length(esh.birds)),
           rep("Amphibians", length(esh.amps)))
# esh.all <- esh.amps
...
already.have <-
  list.files(path = paste0(getwd(),"/ESH_RCPs/Mod_Accuracy/"),full.names = FALSE) %>%
  .[grepl('SSP1',.)] %>%
  gsub(".*Accuracy_","",.) %>%
  gsub(".csv","",.) %>%
  gsub("_Amphibians",'',.) %>%
  ...
  gsub("_Mammals",'',.) %>%
  gsub('_SSP1.*','',.) %>%
  .[!grepl('Model_Thresholds_',.)]


cat('Number Species Completed:',length(already.have),'\n')
...
# And limiting to species endemic to biodiversity hotspots
esh.tot <-
  esh.tot %>%
  filter(species_name %in% species_by_biodiv_hotspot$species)
...
if('Cell_Count' %in% list.files(paste0(getwd(),'/Other Data Inputs'))) {
  ...
  write.csv(esh.tot,paste0(getwd(),'/Other Data Inputs/Cell_Count/Number_of_cells_by_species.csv'),row.names=FALSE)
}
...
sdm_wrap <- function(k) {
  ...
  # Getting proportion of species' habitat range in biodiversity hotspot
  prop_in_hotspot <-
    species_by_biodiv_hotspot %>%
    filter(taxa %in% df$taxa[k] &
             species %in% df$species_name[k])

    prop_in_hotspot <- prop_in_hotspot$prop_in_hotspot
  

  # Printing whether we need to model data for the species
  if(prop_in_hotspot >= 1) {cat(df$species_name[k], round(prop_in_hotspot*100,2), '% Habitat In Biodiversity Hotspots\n')
  } else {cat(df$species_name[k],'Not Proceeding',round((1-prop_in_hotspot)*100,2),' % Habitat Outside of Biodiversity Hotspots \n')}
  
  # Rechecking whether we have the species
  # Doing this becuase order of species is randomised
  # So this will help avoid recreating SDMs for species where these already exist
  already.have <-
    list.files(path = paste0(getwd(),"/ESH_RCPs/Mod_Accuracy/"),full.names = FALSE) %>%
    .[grepl('SSP1',.)] %>%
    gsub(".*Accuracy_","",.) %>%
  ...
  # And check to see whether we need the species
  if(prop_in_hotspot >= .99 &
     !(df$species_name[k] %in% already.have)) {
    ...
    if(esh.check >= 25) { # If species' habitat contains more than 50 cells - run the models
      ...
      eco.keep <- 
        data.frame(esh = getValues(esh.raster),
                   eco_values = getValues(tmp.eco)) %>%
        filter(esh %in% 1) %>%
        filter(eco_values > 1) %>%
        mutate(prop = 1/nrow(.)) %>%
        dplyr::group_by(eco_values) %>%
        dplyr::summarise(prop = sum(prop)) %>%
        filter(prop >= .01)
      eco.keep <- eco.keep$eco_values
      ...
      if(esh.raster@ncols * esh.raster@nrows < 25 | length(eco.keep) %in% 0) {
        ...
        if(!length(test.mods) > 1 | max(test.acc[test.acc < 1]) < 0.75) { # Poorly fitting models
          ...
          for(ssp in ssp.list[c(2)]) {
            ...
            for(year in year.list[!grepl('-19|2100',year.list)]) {
              cat(ssp,year, '\n')
              ...
              cat('End Importing Climate Rasters\n') 
              ...
              if(length(test.mods) > 1) {
                ...
                if(ssp %in% ssp.list[2] &
                  ...
                  tmp.eco.fun <- function(i) {if(sum(unique(getValues(crop(tmp.eco,i))) %in% eco.keep[eco.keep>1]) >= 1) {return(i)}}
  ...
  } # End If statement to check on whether species is endemic to the atlantic forest
} # End SDM WRAP FUNCTION statement
...
df <- 
  ...
  filter(species_name %in% species_by_biodiv_hotspot$species) %>%
  #filter(species_name %in% coo.dat$species) %>%
  #	filter(grepl('Phylloscartes_oustaleti',species_name)) %>%
  sample_n(nrow(.))
#	arrange(no_cells)  %>% 
#	filter(no_cells < 10000000)
# filter(grepl('Bird',taxa)) %>%
# filter(!grepl('Phyllastrephus_albigula',species_name))

# print(head(df$species_name),10)
set.seed(19)
cat(nrow(df),'\n')
#for(ss in 1:nrow(df)) {
#	cat(ss,'\n')
...
# chunk_list <- chunk2(1:nrow(df),10)
# 
# mclapply(chunk_list[[1]],sdm_wrap,mc.cores=5)

...
# List of all species
species.list <-
  do.call(c,lapply(paste0(getwd(),'/ESH_RCPs/SSP5-8.5/',c('Amphibians','Mammals','Birds','Reptiles')),
                   list.files,
                   full.names = TRUE)) %>%
  .[grepl('csv',.)] %>%
  .[grepl('completed',.)] %>%
...
years.loop <-
  list.files(paste0(getwd(),'/ESH_RCPs/SSP5-8.5/Amphibians')) %>%
...
for(i in species.need.list[[chunk_n]]) { # For testing; chunk_n is a variable that indicates which chunk to loop over - doing this so it is easier to change the script as needed
    ...
    eco.keep <- 
      ...
      .[.>1] # this last is a double check to remove oceans
       ...
       write.csv(data.frame(species = tmp.name, taxa = taxa, completed = 'yes'),
                 paste0(getwd(),'/ESH_RCPs/SSP5-8.5/',taxa,'/',tmp.name,'_migrate.csv'))

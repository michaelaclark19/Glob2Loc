...
esh.loss.function <-
  function(current,
    ...
    store.urb.2010 <- species.df$urban.2010.values
    store.urb.values <- species.df$urban.values
    ...
    species.df$urban.2010.values[is.na(species.df$urban.2010.values)] <- 0
    species.df$urban.values[is.na(species.df$urban.values)] <- 0
    ...
    if('urbanization' %in% stresses) {
      species.df$esh.urb.abs <-
        species.df$esh.values -
        species.df$urban.values * species.df$urb.intens.tmp
    }
    ...
    if(current == 'yes') {
      ...
      species.df$esh.current.extent.esh <-
        ...
        species.df$crop.2010 * species.df$crop.intens.2010 -
        species.df$past.2010 * species.df$past.intens.2010 -
        species.df$urban.2010.values * species.df$urb.intens.2010
    ...
    species.df$urban.2010.values <- store.urb.2010
    species.df$urban.values <- store.urb.values
...
pop_estimate_spatial = 
  function(density_raster,
    ...
    tmp.df = data.frame(density = getValues(density_raster),
      ...
      mutate(count = 1) %>%
      rowsum(., group = .$patch_no_migrate, na.rm=TRUE) %>%
      dplyr::select(.,patch_no_migrate, estimated_population, count, habitat_availability_sqkm) %>%
      mutate(patch_no_migrate = patch_no_migrate / count) %>%
      # filter(patch_no_migrate > 0) %>%
...
dispersal_distance_function_old <-
    ...
    if(taxon %in% 'Amphibians') {
      ...
      if(species %in% amp_dispersal_distance_frame) {
        ...
        dispersal_distance = mean(amp_dispersal_distance_frame$Distance[amp_dispersal_distance_frame$Family %in% species.frame$family[species.frame$Species %in% species]])
	      ...
	      # if no matching info - take 90th percentile across amphibians
        dispersal_distance = quantile(amp_dispersal_distance_frame$Distance,.9)
      }
      ...
      if(max(dispersal_distance) < 0) {
        ...
        dispersal_distance = quantile(mam_dispersal_distance_frame$estimated_median_dispersal_m,.9) 

      }
    } else if (taxon %in% 'Birds') {
  
  # setting temporary dispersal distance to gap fill
  dispersal_distance <- -1
  
  # importing csv
  bird_dispersal <- read.csv(paste0(getwd(),'/Other Data Inputs/Patch_Dispersal_Distances/bird_dispersal_distances.csv'), stringsAsFactors = FALSE)
  
  # Based on taxonomies
  # And filling gaps, where needed
  dispersal_distance <-
    bird_dispersal %>%
    mutate(Scientific_Name = paste0('Birds/',gsub(' ','_', Scientific_Name))) %>%
    filter(Scientific_Name %in% species.frame$Species[k]) %>%
    dplyr::select(dispersal_distance) %>%
    as.numeric()
  
  # Gap checking based on genus/family/etc
  # First on genus
  if(max(dispersal_distance) < 0 | is.null(dispersal_distance) | is.na(dispersal_distance)) {
    tmp_dispersal <-
      bird_dispersal %>%
      filter(tolower(Genus) %in% tolower(species.frame$genus[k]))
    
    dispersal_distance <- median(tmp_dispersal$dispersal_distance_genus,na.rm=TRUE)
    
    # if(is.na(dispersal_distance)) {
    #   dispersal_distance <- mean(tmp_dispersal$dispersal_distance,na.rm=TRUE)
    # }
  }
  ...
  if(max(dispersal_distance) < 0 | is.null(dispersal_distance) | is.na(dispersal_distance)) {
    tmp_dispersal <-
      bird_dispersal %>%
      filter(tolower(Family) %in% tolower(species.frame$family[k]))
    
    dispersal_distance <- median(tmp_dispersal$dispersal_distance_family,na.rm=TRUE)
  ...
  if(max(dispersal_distance) < 0 | is.null(dispersal_distance) | is.na(dispersal_distance)) {
    tmp_dispersal <-
      bird_dispersal %>%
      filter(tolower(Order) %in% tolower(species.frame$order[k]))
...
dispersal_distance_function <-
  function(species, species.frame) {
    ...
    } else if (taxon %in% 'Birds') {# setting temporary dispersal distance to gap fill
      dispersal_distance <- -1
...
species.traits.function <-
  function(i) {
    ...
    # Updating estimates so everything is in response to 1
    # E.g. 1 = no effect of land use on species
    # > 1 = positive effect of land use on species
    # < 1 = negative effect on species
    species.traits <-
      species.traits %>%
...
aoh.habs <-
  aoh.habs %>%
  mutate(taxon_merge =
           case_when(Taxon == 'Amphibia' ~ 'Amphibians',
                     Taxon == 'Aves' ~ 'Birds',
                     Taxon == 'Mammalia' ~ 'Mammals',
                     Taxon == 'Reptilia' ~ 'Reptiles')) %>%
  mutate(species = paste0(taxon_merge,'/',species))
...
### Function for checking realm biome values in the sdm maps
# Assumes future habitat range needs to be within same realms and biomes in which species currently exists
# If future habitat range does not do this, then old SDM map is moved to new folder, and file is overwritten with updated map
realm_biome_check <-
  function(aoh_file, 
           sdm_map_raster,
           sdm_raster_map_file,
    ...
    # Importing files
    tmp_aoh <- raster(aoh_file)
    tmp_eco <- raster::crop(raster(paste0(getwd(),'/Ecoregions_Feb2023/biomes_realms_raster.tif')), tmp_aoh) # Importing ecoregion file tmp_aoh)
    # tmp_eco <- raster::crop(raster('/Users/michael/Desktop/Research/Glob2Loc/Ecoregions_Feb2023/biomes_realms_raster.tif'), tmp_aoh)
    
    # Realm biome values in current aoh
    # value of 1 corresponds with ocean - we're only dealing with terrestrial vertebrates
    tmp_eco_aoh_values <- unique(getValues(tmp_eco)[getValues(tmp_aoh) %in% 1]) %>% .[.>1] %>% .[.!=48] # value of 1 corresponds with oceans, value of 48 corresponds with african great lakes

    # Check realm biome values in sdm
    tmp_eco <- raster::crop(raster(paste0(getwd(),'/Ecoregions_Feb2023/biomes_realms_raster.tif')), sdm_map_raster) # Importing ecoregion file tmp_aoh)
    # tmp_eco <- raster::crop(raster('/Users/michael/Desktop/Research/Glob2Loc/Ecoregions_Feb2023/biomes_realms_raster.tif'), sdm_map_raster)
    tmp_eco_sdm_values <- unique(getValues(tmp_eco)[getValues(sdm_map_raster) %in% 1]) # Leaving 1 in here in case oceans have not previously been removed
    
    # If realm biome values in sdm are subset of realm biome values in aoh
    # Then don't need to do anything
    if(sum(tmp_eco_sdm_values %in% tmp_eco_aoh_values) == length(tmp_eco_sdm_values)) {
      # Do nothing - values in realm biome values in sdm are subset of realm biome values in aoh
      # just returning raster
      return(sdm_map_raster)
    } else { # Need to remove extraneous realm biome values
      
      # Removing extraneous realm biome values
      sdm_map_raster[!(tmp_eco %in% tmp_eco_aoh_values)] <- NA
      
      # Moving old raster map file
      # file.copy(from = sdm_raster_map_file,
      #          to = gsub(species_taxa,
      #                    paste0(species_taxa,'_Updated'),
      #                    sdm_raster_map_file))
      # 
      # Saving new raster map in an updated folder
      #raster::writeRaster(sdm_map_raster,
      #            gsub(species_taxa,
      #                    paste0(species_taxa,'_Updated'),
      #                    sdm_raster_map_file),
      #            overwrite = TRUE)
...
# migration function - used for searches and names of files to save
climate_migration_function <-
  function(migration) {

    migration_out <- list('sdm.species.search', # used ~ L177
                          'species_completed.search', # used ~L449
                          'species_completed_year.search', # used ~L610
                          'sdm_map_file.search', # used ~L657 and ~L720
                          'climate_file_save') # term used to save file
    # if limits on climate migration
    if(migration %in% 'limits') {
      migration_out['sdm.species.search'] <- '_migrate.*'
      migration_out['species_completed.search'] <- '[0-9]_completed'
      migration_out['sdm_map_file.search'] <- 'migrate'
      migration_out['species_completed_year.search'] <- '_completed.*'
      migration_out['climate_file_save'] <- ''

    } else {
      migration_out['sdm.species.search'] <- '_migrate.*'
      migration_out['species_completed.search'] <- 'nomigration'
      migration_out['sdm_map_file.search'] <- 'threshold'
      migration_out['species_completed_year.search'] <- '_nomigrationlimits_completed.*'
      migration_out['climate_file_save'] <- '_nomigrationlimits'
...
# Function to gap fill body mass info, for species which this is not available
body_mass_update <-
  function(df) {
    ...
    df_genus <-
      df %>%
      dplyr::group_by(taxon, order, family, genus) %>%
      dplyr::summarise(Genus_Mass_kg = median(Genus_Mass_kg, na.rm = TRUE),
                       Family_Mass_kg = median(Family_Mass_kg, na.rm = TRUE)) #%>%
      # filter(!is.na(genus)) %>%
      # filter(!is.na(Genus_Mass_kg))

    # Adding info to df
    for(i in which(is.na(df$est_mass_kg))) {

      if(!is.na(df$genus[i])) {
        if(sum(!is.na(df_genus$Genus_Mass_kg[df_genus$genus %in% df$genus[i]]))>0) {
          # Updating body mass - est, genus, and then family
          df$est_mass_kg[i] <- mean(df_genus$Genus_Mass_kg[df_genus$genus %in% df$genus[i] & df_genus$taxon %in% df$taxon[i]],na.rm=TRUE)
          df$Genus_Mass_kg[i] <- mean(df_genus$Genus_Mass_kg[df_genus$genus %in% df$genus[i] & df_genus$taxon %in% df$taxon[i]],na.rm=TRUE)
          df$Family_Mass_kg[i] <- mean(df_genus$Family_Mass_kg[df_genus$genus %in% df$genus[i] & df_genus$taxon %in% df$taxon[i]],na.rm=TRUE)

          # And updating order and family
          df$order[i] <- unique(df_genus$order[df_genus$genus %in% df$genus[i] & df_genus$taxon %in% df$taxon[i]]) %>% .[!is.na(.)]
          df$family[i] <- unique(df_genus$family[df_genus$genus %in% df$genus[i] & df_genus$taxon %in% df$taxon[i]]) %>% .[!is.na(.)]
        }
      }
    }

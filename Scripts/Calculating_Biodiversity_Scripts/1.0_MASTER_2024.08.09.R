...
source(paste0(getwd(),"/Scripts/Calculating_Biodiversity_Scripts/0.0_ESH_Loss_Functions_24Nov2023.R"))
source(paste0(getwd(),"/Scripts/Calculating_Biodiversity_Scripts/0.0_Patch_Connectivity_Functions.R"))
...
species.have <-
  do.call(c,
          lapply(taxa.list,
                 function(i) {
                   list.files(paste0(getwd() %>% gsub('ouce-glob2loc','pubh-glob2loc',.),'/Outputs/CSV_File_Outputs/BAU/',i),
                              pattern = 'completed') %>%
                     paste0(i,'/',.)}))
...
species_completed <-
  do.call(c,
          lapply(
            list.files(paste0(csv.estimate.dir %>% gsub('ouce-glob2loc','pubh-glob2loc',.),'/BAU'),
                       full.names = TRUE),
...
cat('Number Species Completed:', length(species_completed))
# Start loop
for(i in scenarios.list) { # Loop through scenarios
  ...
  for(ssp in ssp.list[2]) {
    ...
    for(y in c(2050)) { # For testing
      ...
      species.frame <-
        species.frame.full %>%
        filter(grepl('Brachyc|Ischno',Species))
	#filter(!(Species %in% species_completed_year))#%>%
        #filter(grepl('Leopardus',Species))
        # filter(grepl('Leopardus_wiedii',Species))
     cat('Number Species:',nrow(species.frame)) 
      # For testing
      ...
      if(nrow(species.frame) > 0) { # If statement seeing if any species need to be analysed
        for(k in 1:nrow(species.frame)) {
          ...
          # Progress tracking 
          cat(paste(species.frame$Species[k],"\n"))
	    ...
	    cat('Binding Density Data Frames \n')
	      ...
	      species_df_pop <- species_df_population_noclimate
              ...
              #cat('ESH Values Current: ', sum(species_df_noclimate$esh.values,na.rm=TRUE),'\n')
              #cat('ESH Values Future: ',sum(species_df_climate$esh.values,na.rm=TRUE),'\n')
              #cat('ESH Values Used: ', sum(species_df$esh.values,na.rm=TRUE),'\n')
              
              # Running a function from python
              # This function does a few things
              # Makes two matrices
              # (a) proportion of cell available for habitat
              # (b) cells available for migration
              # And then updates patches based on connectivity,
              ...
              py_run_string("im = r.species_df.loc[:,r.scen_r].values") # Creating rasters with ones and 0s
              ...
              # plot(raster(py$labim))
              # plot(raster(py$labim_no_migrate))
              
              ### Turning arrays to rasters to work with later functions
              # These are used as inputs in later functions
              
              # Habitat area raster
              species.raster.loss = raster(py$im, crs = crs(sdm_map))
              extent(species.raster.loss) = extent(sdm_map)
              
              # Habitat patch, no migration raster
              new.patch_no_migrate = raster(py$labim_no_migrate, crs = crs(sdm_map))
              ...
              # writeRaster(scen_density_raster,paste0('/data/ouce-glob2loc/pubh0329/glob2loc_checks/density_',scen_r,'_',species.frame$binomial[k],'.tif'),overwrite=TRUE)
              # writeRaster(density_raster_scen,paste0('/data/ouce-glob2loc/pubh0329/glob2loc_checks/Scen_Density_Raster_BeforePopChecks',scen_r,'_',y,'.tif'),overwrite=TRUE)
              # writeRaster(species.raster.loss,paste0('/data/ouce-glob2loc/pubh0329/glob2loc_checks/hab_availability_',scen_r,'_',species.frame$binomial[k],'.tif'),overwrite=TRUE)
              ### Population abundance, saving in a data frame
              # By habitat patch
              # and by cells not in a habitat (<20% suitable land cover)
              save.df.spatial.pop <-
                pop_estimate_spatial(density_raster = scen_density_raster,
                                     patch_no_migrate = new.patch_no_migrate,
                                     patch_migrate = new.patch,
              ...
              # Checking population abundance estimates
              cat('Estimated Population',species.frame$Species[k],': ',sum(save.df.spatial.pop$estimated_population),na.rm=TRUE)
              cat('\n')
              ...
              # Creating rasters to save for plotting
              # Only need to do this if there are population estimates for the species
              # Making raster for no migration ----
              if(max(as.numeric(save.df.spatial.pop$estimated_population), na.rm = TRUE) > 0) {
                
                tmp_df_migrate <-
                  left_join(data.frame(patch = getValues(new.patch_no_migrate)),
                            dplyr::select(save.df.spatial.pop,patch = patch_no_migrate, estimated_population))
                
                no.migrate.raster = raster(matrix(tmp_df_migrate$estimated_population, byrow = TRUE, nrow = sdm_map@nrows), crs = crs(sdm_map))
                extent(no.migrate.raster) = extent(sdm_map)  
                
                # Making raster for migration ----
                tmp_df_migrate <-
                  left_join(data.frame(patch = getValues(new.patch)),
                            dplyr::select(save.df.spatial.pop,patch = patch_migrate, estimated_population) %>%
                              group_by(patch) %>% dplyr::summarise(estimated_population = sum(estimated_population, na.rm = TRUE)) %>% unique())
                
                migrate.raster = raster(matrix(tmp_df_migrate$estimated_population, byrow = TRUE, nrow = sdm_map@nrows), crs = crs(sdm_map))
                extent(migrate.raster) = extent(sdm_map)
                
                # Expanding extent of rasters
                # This makes sure the rasters aggregate properly (this is done below)
                # Aggregating rasters loses some resolution
                # But saves a bunch of memory space
                if(no.migrate.raster@nrows < 15) {
                  no.migrate.raster <- 
                    raster::extend(no.migrate.raster, (extent(no.migrate.raster) + c(-22500,22500,-22500,22500)))
                  migrate.raster <- 
                    raster::extend(migrate.raster, (extent(migrate.raster) + c(-22500,22500,-22500,22500)))
                  species.raster.loss <- raster::extend(species.raster.loss, (extent(species.raster.loss) + c(-22500,22500,-22500,22500)))
                } # End if statement to extend migration rasters. Need to do this so that they properly aggregate
                
                # Aggregating rasters to save space
                raster::aggregate(no.migrate.raster, fact = 15, fun = 'max', na.rm = TRUE)
                raster::aggregate(migrate.raster, fact = 15, fun = 'max', na.rm = TRUE)
                
                cat('Saving Population by Patch Rasters\n')
                # And saving rasters that contain the population estimates by habitat patch
                if(max(c(maxValue(no.migrate.raster),maxValue(migrate.raster)),na.rm=TRUE) > 0) {
                  # writeRaster(no.migrate.raster,
                  #             paste0(patch.estimate.dir,"/",i,"/",
                  #                    species.frame$Species[k],
                  #                    "_est_patch_pop_size_year_no_migrate_",
                  #                    y,'_',scen_r,'_',t,'_',ssp,
                  #                    migration['climate_file_save'],
                  #                    '.tif'),
                  #             overwrite = TRUE)
                  # 
                  # writeRaster(migrate.raster,
                  #             paste0(patch.estimate.dir,"/",i,"/",
                  #                    species.frame$Species[k],
                  #                    "_est_patch_pop_size_year_migrate_",
                  #                    y,'_',scen_r,'_',t,'_',ssp,
                  #                    migration['climate_file_save'],
                  #                    '.tif'),
                  #             overwrite = TRUE)
                  
                  rm(no.migrate.raster, migrate.raster)
                } # End if statement to save population rasters by habitat patch
              } # End if statement to make and save spatial population rasters - don't make if no population estimates!
              
              
              # These are remaining habitat estimates by cell
              names(species.raster.loss) <- paste0(scen_r,'_species raster loss year_',y)
              
              # Saving habitat remaining raster...
              cat("Saving Species Raster Loss\n")
              #species.raster.loss
              # writeRaster(species.raster.loss,
              #             paste0(raster.write.dir,"/",i,"/",
              #                    species.frame$Species[k],"_est_habitat_left_year_",
              #                    scen_r,
              #                    '_',y,'_',t,'_',ssp,
              #                    migration['climate_file_save'],
              #                    '.tif'),

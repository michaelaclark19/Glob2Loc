...
source(paste0(getwd(),"/Scripts/Calculating_Biodiversity_Scripts/0.0_ESH_Loss_Functions_2024.08.22_Climate_Suitable.R"))
    ...
    if(species.frame$Species[k] %in% species_tmp_check) {
      ...
      for(t in thresh.list) {
        ...
        for(scen in scen.list) { # for testing
          ...
          writeRaster(scen_density_raster,paste0('/data/ouce-glob2loc/pubh0329/cheetah_climate_suitable_pop_density_',species.frame$binomial[k],'_',t,'_',y,'_',scen_r,'.tif'),overwrite=TRUE)
          
          ag_int_raster <- species_df_pop[,'crop.intens.tmp']
          ag_int_raster <- raster(matrix(ag_int_raster,nrow = sdm_map@nrows,byrow = TRUE))
          crs(ag_int_raster) <- crs(sdm_map)
          extent(ag_int_raster) <- extent(sdm_map)
          
          writeRaster(ag_int_raster,paste0('/data/ouce-glob2loc/pubh0329/cheetah_ag_int_raster_',species.frame$binomial[k],'_',t,'_',y,'_',scen_r,'.tif'),overwrite=TRUE)
